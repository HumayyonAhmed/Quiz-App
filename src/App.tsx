...
const App = () => {
  ...
  return (
    ...
    <Container className={`mt-5 ${!gameOver && !loading ? "" : "new"}`}>
      <h1 className={`text-center display-4 ${!gameOver && !loading ? "text-white" : ""}`}>Quiz</h1>
      <hr/>
      { gameOver ?  
        <>
        ...
        : null}
          ...
          {userAnswers[number] ? <Card.Footer><Button color="primary" onClick={nextQuestion}>{buttonText==="Restart" ? <span>&#8635;</span> : null} {buttonText} {buttonText!=="Restart" ? <span>&#8594;</span> : null}</Button></Card.Footer>:null}
            </Card> : null
          }
        </Container>
  );
